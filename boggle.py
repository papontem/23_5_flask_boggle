...
class Boggle():
    """
    Step#1 Planing and Reading Code
    read first four functions of this class, ignore find and find_from
    PAM: understood! doing....
    """ 
    def __init__(self):
        """
        PAM: contructor initializer for boogle game, creates the games valid words list.
        """
    ...
    def read_dict(self, dict_path):
        ...
        PAM: given a path, will strip word out of each line of a text file and add them to a list. 
        PAM: Returns a list with each element being a string.
        """
    ...
    def make_board(self):
        ...
        board = []
        # string.ascii_uppercase is a string with all uppercase letters of the alphabet.
        for y in range(5):
    ...
    def check_valid_word(self, board, word):
        ...
        PAM: returns a string from the possibl three: 'ok' , 'not-on-board' and 'not-word'
         - 'ok' means its a valid word through and through.
         - 'not-on-board' means the word is in our dictionary but its not on the board.
         - 'not-word' means the word was made up.
        """

        word_exists = word in self.words
        valid_word = self.find(board, word.upper())
        # on_board = self.find(board, word.upper())
        # PAM: better name would be on_board for this variable

        if word_exists and valid_word:
        # if word_exists and on_board:
            result = "ok"
        elif word_exists and not valid_word:
        # elif word_exists and not on_board:
            result = "not-on-board"
        else:
            result = "not-word"
